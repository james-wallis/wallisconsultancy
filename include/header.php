<!-- Header Start -->
<header id="header">
	 <!-- Header Top Bar Start -->
	 <div class="top-bar hidden-sm hidden-xs">
			<div class="slidedown collapse">
				 <div class="container">
						<div class="phone-email pull-left">
							 <a><i class="fa fa-phone"></i> Call Me : 07891760244</a>
							 <a href="mailto:mw@wallisconsultancy.co.uk"><i class="fa fa-envelope"></i> Email : mw@wallisconsultancy.co.uk</a>
						</div>
						<div class="pull-right">
							 <ul class="social pull-left">
									<li class="linkedin"><a href="https://www.linkedin.com/in/mike-wallis-b82a9140/" target="_blank"><i class="fa fa-linkedin"></i></a></li>
							 </ul>
						</div>
				 </div>
			</div>
	 </div>
	 <!-- Header Top Bar End -->
	 <!-- Main Header Start -->

	 <!-- main-header class is used in sticky header -->
	 <div class="main-header">
			<div class="container">
				 <!-- TopNav Start -->

				 <!-- TopNav End -->
				 <!-- Logo Start -->
				<div class="col-lg-8 col-md-8 col-xs-12 col-sm-10">
					<a href="/">
						<div class="logo pull-left">
							<div class="col-lg-2 col-md-2 hidden-xs col-sm-2">
								<img src="/images/mike-wallis-head.png" alt="Mike Wallis"/>
							</div>
							<div class="-text col-lg-10 col-md-10 col-xs-12 col-sm-10">
								<h5>Wallis Consultancy</h5>
								<p>A <em>personal</em> service from a retired senior banker and family magistrate</p>
							</div>
						</div>
					</a>
				</div>
				<div class="hidden-lg hidden-md col-xs-12 col-sm-2">
					<!-- Mobile Menu Start -->
					<div class="mobile navbar-header">
							<a class="navbar-toggle" data-toggle="collapse" href=".navbar-collapse">
							<i class="fa fa-bars fa-2x"></i>
							</a>
					</div>
				</div>
				 <!-- Mobile Menu End -->
				 <!-- Logo End -->
				 <!-- Mobile Menu Start -->

				 <!-- Mobile Menu End -->
				 <!-- Menu Start -->
				 <div class="col-lg-4 col-md-4 col-xs-12 col-sm-12">
					<nav class="collapse navbar-collapse menu">
							<ul class="nav navbar-nav sf-menu">
								<li id="<?php if (substr($atab, 0, 3)=="hom") {echo "current";} ?>">
										<a href="/" >
										Home
										</a>
								</li>
								<li id="<?php if (substr($atab, 0, 3)=="med") {echo "current";} ?>">
										<a href="/mediation/index.html">
										Areas of Mediation
										</a>
								</li>
							</ul>
					</nav>
				</div>
				 <!-- Menu End -->
			</div>
	 </div>
	 <!-- Main Header End -->
</header>
<!-- Header End -->
